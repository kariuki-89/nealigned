...
function cdn_listenForSnapshots_messages(isFirst="no") {
  ...
  if (user) {
    ...
    let itemsCollection
    if(first=="yes"){*/
    const itemsCollection = db.collection("messages").orderBy("createdAt", "asc");
    const lastItemQuery = itemsCollection
    /*}
    else{*/
    ...
    lastItemQuery.onSnapshot((snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        if(doc.data().agent==theAgentId&&moveNext==true){
